...
<div style="display: flex; flex-direction: column; gap: 20px; padding-top: 4px;">
  ...
  <div class="page-header">
    <div style="display: flex; align-items: center; justify-content: space-between;">
      <div>
        <h1 class="page-title" style="font-size: 22px;">Halo, <?= esc($tentor['name'] ?? 'Tentor') ?>!</h1>
      ...
      <div style="width: 44px; height: 44px; border-radius: 14px; background: var(--warm-cream); display: flex; align-items: center; justify-content: center; border: 1px solid var(--border-light);">
        <span style="font-family: 'Outfit', sans-serif; font-weight: 700; color: var(--dark-navy); font-size: 16px;">
          <?= strtoupper(substr($tentor['name'] ?? 'T', 0, 1)) ?>
        </span>
      </div>
    </div>
  </div>

  <!-- Role Notification & Access Restriction Banner -->
  <div class="card" style="padding: 16px; background: #FFFBEB; border-color: #FDE68A; box-shadow: none;">
    <div style="display: flex; gap: 12px; align-items: flex-start;">
      ...
      <div style="display: flex; flex-direction: column; gap: 4px;">
        <span style="font-size: 13px; font-weight: 700; color: #92400E;">Workspace Mandiri Guru / Tentor</span>
        <p style="font-size: 12px; color: #78350F; line-height: 1.5; margin: 0;">
          Anda sedang mengelola data sebagai <strong><?= esc($tentor['name'] ?? 'Tentor') ?></strong>. Bank soal, jadwal sesi bimbel, dan data nilai siswa yang tampil di akun Anda terisolasi khusus untuk materi dan murid bimbingan Anda.
        </p>
      </div>
    </div>
  </div>

  <!-- Quick Stats (4 Isolated Metrics) -->
  <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 10px;">
    <div class="card" style="padding: 14px;">
      <span style="font-size: 11px; font-weight: 600; color: var(--text-muted); text-transform: uppercase; letter-spacing: 0.5px;">Paket Soal Saya</span>
      <span style="font-family: 'Outfit', sans-serif; font-size: 24px; font-weight: 700; color: var(--dark-navy); margin-top: 2px;">
        <?= $totalPackages ?? 0 ?>
      </span>
    ...
    <div class="card" style="padding: 14px;">
      <span style="font-size: 11px; font-weight: 600; color: var(--text-muted); text-transform: uppercase; letter-spacing: 0.5px;">Butir Soal Saya</span>
      <span style="font-family: 'Outfit', sans-serif; font-size: 24px; font-weight: 700; color: var(--dark-navy); margin-top: 2px;">
        <?= $totalQuestions ?? 0 ?>
      </span>
      <span style="font-size: 11px; color: var(--text-muted);">Materi buatan Anda</span>
    </div>

    <div class="card" style="padding: 14px;">
      <span style="font-size: 11px; font-weight: 600; color: var(--text-muted); text-transform: uppercase; letter-spacing: 0.5px;">Jadwal Bimbel</span>
      <span style="font-family: 'Outfit', sans-serif; font-size: 24px; font-weight: 700; color: var(--warm-amber); margin-top: 2px;">
        <?= $totalSchedules ?? 0 ?>
      </span>
      <span style="font-size: 11px; color: var(--text-muted);">Sesi kelas aktif</span>
    </div>

    <div class="card" style="padding: 14px;">
      <span style="font-size: 11px; font-weight: 600; color: var(--text-muted); text-transform: uppercase; letter-spacing: 0.5px;">Murid Bimbingan</span>
      <span style="font-family: 'Outfit', sans-serif; font-size: 24px; font-weight: 700; color: var(--soft-sage-green); margin-top: 2px;">
        <?= $totalStudents ?? 0 ?>
      </span>
      <span style="font-size: 11px; color: var(--text-muted);">Siswa Anda</span>
    </div>
  </div>
